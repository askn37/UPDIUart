...
これは megaAVR0 / tinyAVR0,1,2 / AVRDA,DB 系統に採用されている UPDI通信に必要なホスト機能を、megaAVR-0自身の上に実装することを支援するライブラリである。
...
ゆえにこのライブラリを使用するには、megaAVR-0 / tinyAVR-0,1,2 / AVR DA,DB 系統のいずれかの MCUを必要とする。特にホスト側については必要十分な記憶容量を要求する。

一方でターゲット側 MCUに AVR DA,DB 系統を選ぶのは、0.1版時点では制約がある。これらの品種は NVMCTRL制御手順が異なるため、抽象化メソッド群が満足できていない。低レベル層については使用できる。

- 低レベル層は全種共通なので、AVR DA,DB 系統専用ライタを書く一助にはなる。
